#!/bin/bash
# Iter 12: retain CE + heavy momentum 0.99 (vs default 0.97). Test if extra smoothing
# recovers baseline-level stability with the simplified CE-grad source.
set +e
...
echo "[$(date)] Iter12 pilot: retain CE + retain_momentum=0.99 — Llama Bio"
$BASE $SC \
    ...
    trainer.method_args.cfg.retain_momentum=0.99 \
    +trainer.method_args.cfg.retain_grad_source=ce \
    task_name=iter12_ce_mom099_llama_bio
