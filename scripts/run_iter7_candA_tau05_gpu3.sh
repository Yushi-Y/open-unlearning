#!/bin/bash
# Iter 7A: §8 Candidate A (retain-confidence token mask), tau=0.5, Llama Bio.
set +e
...
echo "[$(date)] Iter7A pilot: retain_conf tau=0.5, no KL filter — Llama Bio"
$BASE $SC \
    ...
    task_name=iter7_candA_tau05_llama_bio
