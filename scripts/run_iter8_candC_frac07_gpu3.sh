#!/bin/bash
# Iter 8: §8 Candidate C (retain-cov-weighted token reject), frac=0.7 (more aggressive).
set +e
...
echo "[$(date)] Iter8 pilot: retain_cov_reject frac=0.7 — Llama Bio"
$BASE $SC \
    ...
    +trainer.method_args.cfg.token_filter_tau=0.7 \
    task_name=iter8_candC_frac07_llama_bio
