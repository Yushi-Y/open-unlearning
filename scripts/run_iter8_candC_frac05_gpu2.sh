#!/bin/bash
# Iter 8: §8 Candidate C (retain-cov-weighted token reject), frac=0.5.
# Replaces KL filter with ||g_t|| * (a^T P_r a) ranking — gradient-aware + retain-cov.
set +e
...
echo "[$(date)] Iter8 pilot: retain_cov_reject frac=0.5 — Llama Bio"
$BASE $SC \
    ...
    +trainer.method_args.cfg.token_filter_tau=0.5 \
    task_name=iter8_candC_frac05_llama_bio

echo "[$(date)] Iter8 pilot DONE"
