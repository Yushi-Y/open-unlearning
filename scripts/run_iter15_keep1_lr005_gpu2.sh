#!/bin/bash
# Iter 15: "keep only #1" LR sweep — CE + mom=0 + per-step, LR=0.05 (quarter default).
set +e
...
echo "[$(date)] Iter15: keep only #1 (CE + mom=0 + per-step) LR=0.05"
$BASE $SC \
    trainer.args.learning_rate=0.05 \
    ...
    task_name=iter15_keep1_lr005_llama_bio
echo "[$(date)] Iter15 LR=0.05 DONE"
