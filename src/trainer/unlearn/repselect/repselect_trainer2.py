...
class RepSelect2(UnlearnTrainer):
    def __init__(self, cfg, *args, **kwargs):
        ...
        if self.cfg.get("forget_loss") == "npo_saturating":
            self.forget_batches = [
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        if self.cfg.get("forget_loss") == "npo_saturating":
            # Per-sequence NLL (sum over tokens)
            ...
            per_seq_nll = per_token_nll.sum(dim=-1)  # [batch_size]

            if "initial_nll" not in batch:
                batch["initial_nll"] = per_seq_nll.detach()

            beta = self.cfg.get("npo_beta", 1.0)
            diff = per_seq_nll - batch["initial_nll"].to(per_seq_nll.device)
            forget_loss = -F.logsigmoid(beta * diff).mean() / beta
        else:
