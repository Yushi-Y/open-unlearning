# python src/train.py --config-name=unlearn.yaml experiment=unlearn/wmdp_low_mi/default trainer=EigenSurgery task_name=test
"""One-shot eigenvalue surgery + optional gradient ascent fine-tuning.

Phase 1 (epoch 0): Collect forget+retain activations, compute DISCO directions
Phase 2 (after epoch 0): Directly subtract selective weight components:
    W_new = W - α * (W @ V @ diag(w) @ V.T)
Phase 3 (remaining epochs): Standard gradient ascent (no collapse) for fine-tuning
"""
import logging
import math
...
class EigenSurgery(UnlearnTrainer):
    ...
    def _do_surgery(self, model):
        ...
        logging.info(f"Performing eigenvalue surgery with α={alpha}")
        with pt.no_grad():
            for module in model.modules():
                if hasattr(module, "disco_collapser") and hasattr(module.disco_collapser, "eig_vec"):
                    V = module.disco_collapser.eig_vec.to(module.weight.dtype)
                    w = module.disco_collapser.weights.to(module.weight.dtype)
                    # W_selective = W @ V @ diag(w) @ V.T
                    proj = module.weight.data @ V
                    weighted = proj * w
                    W_selective = weighted @ V.T
                    module.weight.data -= alpha * W_selective
        self.surgery_done = True
        logging.info("Surgery complete")

    def training_step(self, model, inputs, num_items_in_batch=None):
        model.train()

        # Epoch 0: collect retain + forget activations
    ...
    def collapse_hook(self, module, grad_input, grad_output):
        ...
        if self.batch_idx < self.recalc_every or not self.surgery_done:
            return

        # After surgery: standard gradient (surgery already targeted selective dirs)
