# python src/train.py --config-name=unlearn.yaml experiment=unlearn/wmdp_low_mi/default trainer=DISCOv2 task_name=test
"""DISCO v2: Orthogonalized directions + selectivity filter.

Pure DISCO — no KL masking, no LoRA. Two improvements:
1. Orthogonalized eigenvectors → clean Mahalanobis, less disruption per step
2. DISCO selectivity filter → tokens with mostly-shared activations are filtered
"""
import logging
import math
...
from trainer.unlearn.repcollapse.disco_v2_collapser import DiscoV2Collapser
...
class DISCOv2(UnlearnTrainer):
    def __init__(self, cfg, *args, **kwargs):
        ...
        for layer_num in range(len(self.model.model.layers)):
            ...
            shared_act = DiscoV2Collapser(n_pcs, eps)
            down_act = DiscoV2Collapser(n_pcs, eps)
            shared_grad = DiscoV2Collapser(n_pcs, eps)
            down_grad = DiscoV2Collapser(n_pcs, eps)
            for module in [mlp.gate_proj, mlp.up_proj, mlp.down_proj]:
                ...
                if module is mlp.down_proj:
                    module.act_collapser = down_act
                    module.grad_collapser = down_grad
                else:
                    module.act_collapser = shared_act
                    module.grad_collapser = shared_grad

    def training_step(self, model, inputs, num_items_in_batch=None):
        model.train()

        # Epoch 0: collect retain activations
        if self.batch_idx < self.recalc_every:
            ...
            # Retain backward for gradient DISCO
    ...
    def collapse_hook(self, module, grad_input, grad_output):
        ...
        # DISCO v2 collapse (orthogonalized)
        acts = module.act_collapser.collapse(acts)
        grads = module.grad_collapser.collapse(grads)

        # DISCO selectivity filter: keep only selective tokens
        selectivity = module.act_collapser.token_selectivity(acts)
        threshold = self.cfg.get("selectivity_threshold", 1.0)
        sel_mask = selectivity > threshold
        acts = acts[sel_mask]
        grads = grads[sel_mask]
