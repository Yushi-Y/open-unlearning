...
class DISCO(UnlearnTrainer):
    """Discriminative Collapse: DISCO directions + Mahalanobis whitening.
    Minimal version — collapse acts only, no KL filter, no LoRA."""
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        # Retain activation collection (epoch 1 only, forward only)
    ...
    def collapse_hook(self, module, grad_input, grad_output):
        ...
        # Collapse acts via Mahalanobis (DISCO directions, absolute variance scaling)
        acts = module.disco_collapser.collapse(acts)
